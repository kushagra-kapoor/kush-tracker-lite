...
from views.intraday_monitor import fetch_yfinance_batch, process_intraday_data
...
def run_daily_monitor():
    ...
    c = conn.cursor()
    c.execute('''
        INSERT OR REPLACE INTO daily_journal (date, journal_notes, automated_filled)
        VALUES (?, ?, ?)
    ''', (date_str, ai_briefing, 1))
    conn.commit()
    conn.close()
