...
def process_intraday_data(history_df, tickers, index_data=None, industry_map=None):
    ...
    index_today_pct = market_state['index_pct']
    ...
    # Fetch cross-conviction overlay sets
    try:
    ...
    for ticker, df in ticker_data.items():
        ...
        results.append({
            ...
            'TickerLink': f"https://in.tradingview.com/chart/?symbol=NSE:{clean_ticker}&name={ticker_display}",
            'Industry': industry,
            'Today %': today_pct,
            'Volume Expansion': vol_expansion,
            'Dollar Volume (Cr)': dollar_volume_cr,
            'Close': current_price,
...
def main():
    ...
    with col1:
        ...
        render_header("📡 Intraday Snapshot Monitor", "Live intelligence for NSE Equities")
    ...
    mmi_val = se.get_tickertape_mmi()
    india_vix_val = se.get_india_vix()
    india_pcr_val = se.get_india_pcr()
    
    mac_col1, mac_col2 = st.columns(2)
    
    with mac_col1:
        with st.container(border=True):
            fig_mmi = se.create_gauge(mmi_val, "Tickertape MMI (Fear/Greed)", 0, 100, [30, 70], inverse_colors=False, valueformat=".0f")
            st.plotly_chart(fig_mmi, use_container_width=True)
            if mmi_val is not None:
                if mmi_val < 30:
                    st.markdown("<p style='text-align: center; color: #10b981; font-size: 0.95rem; margin-top:-10px;'>🟢 <b>Extreme Fear:</b> Contrarian opportunity zone. Buy the panic.</p>", unsafe_allow_html=True)
                elif mmi_val > 70:
                    st.markdown("<p style='text-align: center; color: #ef4444; font-size: 0.95rem; margin-top:-10px;'>🔴 <b>Extreme Greed:</b> Caution, market overextended. Trim.</p>", unsafe_allow_html=True)
                else:
                    st.markdown("<p style='text-align: center; color: #e2e8f0; font-size: 0.95rem; margin-top:-10px;'>🟡 <b>Neutral:</b> No extreme sentiment edge. Follow price action.</p>", unsafe_allow_html=True)
            
    with mac_col2:
        with st.container(border=True):
            fig_vix = se.create_gauge(india_vix_val, "India VIX (Volatility)", 10, 30, [15, 22], inverse_colors=False, valueformat=".1f")
            st.plotly_chart(fig_vix, use_container_width=True)
            if india_vix_val is not None:
                if india_vix_val < 15:
    ...
    with pin_cols[0]:
        pin_ticker = st.text_input("⭐ Pin Ticker to Focus List (e.g. RELIANCE.NS):", key="pin_intra_in_text")
    with pin_cols[1]:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("Pin to Focus List", key="btn_pin_intra_in", use_container_width=True):
            if pin_ticker:
                from database import add_to_focus_list
                if add_to_focus_list(pin_ticker.upper().strip(), "IN"):
    ...
    if not top_sectors:
        ...
        st.info(f"💡 **Symbol Guide:** 👑 = True Market Leader &nbsp;&nbsp;|&nbsp;&nbsp; 🎩 = Next 20 Leader &nbsp;&nbsp;|&nbsp;&nbsp; ⚡ = Super Compounder &nbsp;&nbsp;|&nbsp;&nbsp; 🚀 = Rocket Rank &nbsp;&nbsp;|&nbsp;&nbsp; 🔥 = Hot Sector ({', '.join(top_sectors)})")
        
    st.caption("Universe: NSE Equities from `tickers.txt`")
    ...
    with col_u2:
        min_adtv = st.slider(
            ...
            value=5 if st.session_state.get('universe_mode_radio') == "Deep Market (2500+ NSE Stocks)" else 0, 
            step=1, 
            help="Filter out illiquid stocks based on 20-day Average Dollar Volume."
        )
    ...
    if st.session_state.get('refresh_trigger', False):
        st.session_state['refresh_trigger'] = False # Reset
        
        with st.spinner(f"Fetching Universe List & Sectors ({st.session_state.get('universe_mode_radio', 'NIFTY 750 (High Conviction)')})..."):
            selected_mode = st.session_state.get('universe_mode_radio', 'NIFTY 750 (High Conviction)')
            from database import get_all_fundamentals_cache
            db_cache = get_all_fundamentals_cache()
            
            if selected_mode == "NIFTY 750 (High Conviction)":
                tickers, nse_map = fetch_nifty_total_market_tickers(return_industry_map=True)
        ...
        with st.spinner("Computing Intraday Metrics..."):
            ...
            for _, row in results_df.iterrows():
                t = str(row['Ticker']).replace('👑', '').replace('🔥', '').strip()  # clean ticker without emoji
                ...
                # Institutional Footprint (Volume Shock) Check
            ...
            st.session_state['breadth_history_df'] = breadth_history_df
            st.session_state['market_state'] = market_state
            st.session_state['index_pct'] = market_state.get('index_pct', 0.0)
            
            # Update timestamp
            local_tz = pytz.timezone('Asia/Kolkata')
            now_ist = datetime.now(local_tz)
            st.session_state['last_updated'] = now_ist.strftime('%d %b, %H:%M:%S IST')
            st.rerun()
    ...
    if min_adtv > 0:
        results_df = results_df[results_df['Dollar Volume (Cr)'] >= min_adtv].copy()
    ...
    st.markdown("### 🚦 The Market Filter (NIFTY 500 Regime)")
    if qull_green:
        banner_color = "#10b981"
        banner_icon = "🟢"
    ...
    st.caption("Aggregates the momentum of all 750 components to identify macro thematic leadership.")
    ...
    breadth_history_df = st.session_state.get('breadth_history_df', pd.DataFrame())
    if not breadth_history_df.empty:
        ...
        with hc_col1:
            with st.container(border=True):
                ...
                st.caption("**Condition:** Daily % of 750 stocks closing positive. **Use:** Gauge broad market health. **Line:** 10 DMA.")
    ...
    st.caption("Matches institutional momentum rules: Dollar Volume > ₹20Cr, ADR > 4%, 1M Ret > 40% (or >90th Percentile 1M/3M), and Not Extended (>15% above 10EMA). Setup trigger: EP, Breakout, or Launchpad.")
    
    # Apply strict Qullamaggie rules
    qull_df = results_df[
        (results_df['Dollar Volume (Cr)'] >= 20.0) &
        (results_df['ADR %'] >= 4.0) &
    ...
    if qull_df.empty:
        ...
        render_empty_state("No stocks meet the strict Qullamaggie criteria today. Sit on your hands! Cash is a position.", "🧘‍♂️")
    else:
        ...
        display_qull = display_qull[['Ticker', 'TickerLink', 'Setup Trigger', 'Today %', 'Dollar Volume (Cr)', 'ADR %', '1M Return', '1M Ret Pctile', '3M Return', '3M Ret Pctile', 'Dist from 10 EMA %', 'Is Power Trend', 'Is Ants', 'HV1 Defended']].copy()
        ...
        styled_qull = style_dataframe(display_qull[['TickerLink', 'Setup Trigger', 'Today %', 'Dollar Volume (Cr)', 'ADR %', '1M Return', '1M Ret Pctile', '3M Return', '3M Ret Pctile', 'Dist from 10 EMA %', 'Is Power Trend', 'Is Ants', 'HV1 Defended']])
    ...
    if glb_df.empty:
        ...
        display_glb = glb_df[['Ticker', 'TickerLink', 'Today %', 'Volume Expansion', 'Dollar Volume (Cr)', '1M Return', 'Dist from 10 EMA %', 'Is Power Trend', 'Is Ants', 'HV1 Defended']].copy()
        
        tv_text_glb = generate_tv_export(display_glb)
        with st.expander("📋 Copy TradingView List (GLBs)"):
            st.code(tv_text_glb, language="text")
            
        styled_glb = style_dataframe(display_glb[['TickerLink', 'Today %', 'Volume Expansion', 'Dollar Volume (Cr)', '1M Return', 'Dist from 10 EMA %', 'Is Power Trend', 'Is Ants', 'HV1 Defended']])
    ...
    # SEC 5: THE DAVID RYAN 3-WEEKS TIGHT SCANNER
    st.markdown("### 🗜️ SEC 5: The David Ryan 3-Weeks Tight Scanner")
    st.caption("Condition: The variance between the last 3 weekly closing prices is <= 2.0% and the stock is above its 50 SMA. Shows extreme institutional price control and supply dry-up.")
    
    ryan_df = results_df[results_df.get('Is 3WT', False)].copy()
    if ryan_df.empty:
        from components import render_empty_state
        render_empty_state("No 3-Weeks Tight setups detected today.", "🛌")
    else:
        ryan_df = ryan_df.sort_values('1M Return', ascending=False)
        display_ryan = ryan_df[['Ticker', 'TickerLink', 'Today %', 'Volume Expansion', 'Dollar Volume (Cr)', '1M Return', 'Dist from 10 EMA %', 'Is Power Trend', 'Is Ants', 'HV1 Defended']].copy()
        
        tv_text_ryan = generate_tv_export(display_ryan)
        with st.expander("📋 Copy TradingView List (3WT Setups)"):
            st.code(tv_text_ryan, language="text")
            
        styled_ryan = style_dataframe(display_ryan[['TickerLink', 'Today %', 'Volume Expansion', 'Dollar Volume (Cr)', '1M Return', 'Dist from 10 EMA %', 'Is Power Trend', 'Is Ants', 'HV1 Defended']])
    ...
    # SEC 6: ELITE BREAKOUT SETUPS (STAGE 2)
    with col1:
        st.markdown("### 🏆 SEC 6: Stage 2 Elite Breakouts (Minervini)")
        ...
        if elite_df.empty:
            from components import render_empty_state
            render_empty_state("No Stage 2 Elite Breakouts detected today.", "📉")
        else:
    ...
    with col3:
        ...
        if lp_df.empty:
            from components import render_empty_state
            render_empty_state("No Launch Pad setups detected today. Price action is too loose.", "🚀")
        else:
    ...
    with col4:
        ...
        if hv1_df.empty:
            from components import render_empty_state
            render_empty_state("No HV1 signals detected today. Waiting for record-breaking institutional volume.", "🔊")
        else:
    ...
    if htf_df.empty:
        from components import render_empty_state
        render_empty_state("No High Tight Flags (Power Plays) detected today. This is an extremely rare and explosive setup.", "🔥")
    else:
    ...
    with col_kell1:
        ...
        if crossback_df.empty:
            from components import render_empty_state
            render_empty_state("No EMA Crossback bounce setups detected today.", "🏹")
        else:
    ...
    with col_kell2:
        ...
        if rev_df.empty:
            from components import render_empty_state
            render_empty_state("No Reversal Extension (Capitulation) setups detected today.", "🪃")
        else:
    ...
    if reclaim_df.empty:
        from components import render_empty_state
        render_empty_state("No 50 SMA Reclaims detected today.", "🎣")
    else:
