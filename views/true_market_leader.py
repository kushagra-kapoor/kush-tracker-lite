...
def get_cached_universe(mode="NIFTY 750 (High Conviction)"):
    if mode == "NIFTY 750 (High Conviction)":
        return fetch_nifty_total_market_tickers()
    elif mode == "Indian Smart Beta & Sectoral ETFs":
        import os
        file_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'tickers_etf.txt')
        try:
        ...
        import os
        file_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'tickers.txt')
        try:
            with open(file_path, 'r') as f:
                # Some symbols might be illiquid and have dashes or weird formatting
                tickers = [line.strip().upper() for line in f if line.strip() and '-' not in line]
            return list(set(tickers))
        except Exception:
            return []
...
@st.cache_data(ttl=3600, show_spinner=False)
def get_cached_history_fast(tickers_tuple, days):
    return fetch_yfinance_batch(list(tickers_tuple), days=days)

def get_cached_history(tickers, days):
    # Convert list to tuple so Streamlit can hash it for caching
    df = get_cached_history_fast(tuple(tickers) if tickers is not None else (), days)
    return df
...
def run_technical_prescreen(history_df, tickers, rs_scores, is_etf=False):
    ...
    # Download benchmark for RS Blue Dot (Nifty 500)
    try:
        bench_df = yf.download('^CRSLDX', period="252d", progress=False)
        bench_df.columns = [c.lower() for c in bench_df.columns]
    except Exception as e:
        print(f"[TML] Failed to download benchmark index: {e}")
    ...
    for t in tickers:
        ...
        adtv_cr = (vol_21 * close) / 10000000.0
        if not is_etf and (adtv_cr < 5.0):
            continue
        funnel['Highly Liquid (ADTV > ₹5Cr)'] += 1
        ...
        # HV1: Was today's volume (or any of the last 21 days) the highest in 250 days?
        hv1_detected = False
        if len(v_series) >= 250:
            vol_250 = v_series.tail(250)
            max_vol_250 = vol_250.max()
            # Check if any of the last 21 days had the 250-day max volume
        ...
        if len(c_series) >= 51 and len(v_series) >= 50:
            recent_50_closes = c_series.tail(51)  # 51 to get 50 pct_changes
            recent_50_vols = v_series.tail(50)
            daily_rets = recent_50_closes.pct_change().iloc[1:]  # 50 values
            # Align indices
        ...
        if len(c_series) >= 6:
            recent_6_closes = c_series.tail(6)
            if recent_6_closes.iloc[0] > 0:
                ret_1w = ((recent_6_closes.iloc[-1] / recent_6_closes.iloc[0]) - 1) * 100

        if len(c_series) >= 22:
            recent_22_closes = c_series.tail(22)
            if recent_22_closes.iloc[0] > 0:
                ret_1m = ((recent_22_closes.iloc[-1] / recent_22_closes.iloc[0]) - 1) * 100
        ...
        if len(c_series) >= 126:
            recent_126_closes = c_series.tail(126)
            if recent_126_closes.iloc[0] > 0:
                ret_6m = ((recent_126_closes.iloc[-1] / recent_126_closes.iloc[0]) - 1) * 100
        ...
        passed_stocks.append({
            ...
            'VDU_5D': vdu_5d,
            'PocketPivot': pocket_pivot_5d,
            'HV1_Flag': hv1_detected,
            'UD_Ratio': ud_ratio,
            'Vol_3M': vol_3m,
            'RMV_15D': rmv_15d,
            'Ret_1W': ret_1w,
...
def score_leaders(candidates, is_etf=False):
    ...
    Total: 120 points (Growth 40 + Tech 30 + Liquidity 20 + Quality 10 + Institutional 20)
    """
    scored = []
    for c in candidates:
        if is_etf:
        ...
        # Liquidity (Max 10)
        adtv = c['adtv_cr']
        liq_pts = min(10, max(0, (adtv / 100.0) * 10))
        ...
        # HV1 in last 21 days (10 pts)
        hv1_pts = 10 if c.get('hv1', False) else 0
        
        # 50-Day U/D Volume Ratio (10 pts)
...
# -----------------------------------------------------------------------------
def main():
    from components import render_header, render_disk_cache_sidebar
    render_header("👑 True Market Leader Engine", "Locating the Apex Predators of the Indian Market.")
    ...
    with st.expander("📖 True Market Leader Scoring Methodology (120 Points)", expanded=False):
        ...
        **What makes a True Market Leader?**
        ...
        - **Average Daily Trading Value (up to 10 pts)**: TMLs must handle large fund inflows. Scales up to ₹100+ Crores ADTV.
        ...
        - **Highest Volume in 1 Year — HV1 (10 pts)**: If the stock printed its highest volume in 250 trading days at any point during the last 21 sessions, it signals fresh heavyweight accumulation. Binary trigger: 10 or 0.
        - **50-Day Up/Down Volume Ratio (10 pts)**: Measures sustained institutional buying pressure. Ratio ≥ 2.0 = 10 pts (Elite) | 1.5–1.9 = 7 pts (Strong) | 1.1–1.4 = 3 pts (Mild) | < 1.0 = 0 pts.
        ...
        *Engine Pipeline: Dynamically pre-screens 750 NIFTY Total Market stocks, dropping any that fail elite technical baselines (Stage 2, RS > 80, Close > ₹20). Fetches live fundamentals only for survivors.*
        """)
    ...
    if asset_class == "Stocks":
        universe_mode = st.radio(
            "Select Universe:",
            ["NIFTY 750 (High Conviction)", "Deep Market (2500+ NSE Stocks)"],
            horizontal=True
        )
    else:
        universe_mode = st.radio(
            "Select Universe:",
            ["Indian Smart Beta & Sectoral ETFs"],
            horizontal=True
        )
    ...
    with tab_sectors:
        ...
        with r1:
            heat_sort = st.radio("Sort Grid By:", ["RS Rating", "1M Velocity"], horizontal=True)
        with r2:
            weight_mode = st.radio("Weighting Method:", ["Equal", "Market Cap"], horizontal=True)
        with r3:
            min_rs_filter = st.slider("Min RS Rating:", min_value=0, max_value=99, value=0, step=5)
        with r4:
            min_breadth_filter = st.slider("Min Stage 2 Breadth (%):", min_value=0, max_value=100, value=0, step=10)
        ...
        if st.button("🔥 Load Sector Heat Rankings", use_container_width=True):
            with st.spinner("Fetching historical data and computing momentum..."):
                tickers = get_cached_universe(universe_mode)
                ...
                st.session_state.heat_data_in = (history_df, tickers, db_cache, rs_scores)
        
        if 'heat_data_in' in st.session_state:
            history_df, tickers, db_cache, rs_scores = st.session_state.heat_data_in
            render_sector_heat_rankings(history_df, tickers, db_cache, rs_scores, sort_by=heat_sort, min_rs=min_rs_filter, min_breadth=min_breadth_filter, weight_mode=weight_mode)
    with tab_apex:
        ...
        if run_scan:
            ...
            with st.status("Deploying TML Engine...", expanded=True) as status:
                ...
                st.write(f"1. Scanning & Fetching Database for: {universe_mode}")
                tickers = get_cached_universe(universe_mode)
                
                st.write("2. Downloading Cached Matrix Technicals (Last 252 Days)...")
                get_cached_history_fast.clear()
                history_df = get_cached_history(tickers, days=252)
                ...
                if final_leaders:
                    ...
                    save_tml_snapshot("INDIA", final_leaders, top_n=40)
        ...
        if st.session_state.tml_results is not None:
            ...
            st.caption("Jim Roppel says to brutally filter the market noise. Here is how the NIFTY 750 was decimated:")
            ...
            with c_radar:
                ...
                clean_apex_ticker = apex['ticker'].replace('.NS', '')
            ...
            st.info("💡 **Symbol Guide:** &nbsp;&nbsp; 👑 **Apex Predator** &nbsp;&nbsp;|&nbsp;&nbsp; ⚡ **Super Compounder** &nbsp;&nbsp;|&nbsp;&nbsp; 🚀 **Rocket Rank** &nbsp;&nbsp;|&nbsp;&nbsp; 🐺 **Sector Wolfpack** (3+ leaders) &nbsp;&nbsp;|&nbsp;&nbsp; 🐘 **Liq Expansion** &nbsp;&nbsp;|&nbsp;&nbsp; 🛡️ **Buy@21EMA** &nbsp;&nbsp;|&nbsp;&nbsp; 🚨 **Micro Signal**")
            ...
            recent_signals = get_recent_intraday_signals("INDIA", days=3)
            try:
            ...
            top_20_raw = [r['ticker'].replace('.NS', '') for r in leaders_to_show]
            hist_df = get_cached_history([r['ticker'] for r in leaders_to_show], 430)
            ...
            readiness_results = analyze_multibagger_stocks(top_20_raw, pre_fetched_data=pre_fetched)
            readiness_map = {res['ticker']: res.get('buy_readiness', {}) for res in readiness_results}
            
            display_data = []
            for i, rank in enumerate(leaders_to_show):
                clean_ticker = rank['ticker'].replace('.NS', '')
                ...
                ticker_display = f"NSE:{clean_ticker}"
                if is_apex:
                    ticker_display += " 👑"
                if clean_ticker in super_compounders:
                    ticker_display += " ⚡"
                if clean_ticker in rockets:
                    ticker_display += " 🚀"
                ...
                display_data.append({
                    ...
                    'Why?': read_breakdown,
                    'Chart': f"https://in.tradingview.com/chart/?symbol=NSE:{clean_ticker}",
                    ...
                    'ADTV (Cr)': round(rank['adtv_cr'], 1),
                    'Dist High %': round(rank['dist_high'], 1),
                    'ROE %': round(rank.get('roe', 0), 1),
                    'HV1_Flag': rank.get('hv1', False),
                    'PocketPivot': '🌊' if rank.get('Pocket_Pivot_Flags', False) else '',
                    'UD_Ratio': round(rank.get('ud_ratio', 1.0), 2),
                    'VDU_5D': '🏜️' if rank.get('VDU_Flag', False) else '',
                    'RMV_15D': rank.get('RMV_15D', 100),
                    '3WT': rank.get('3WT', False),
                    'Ants': rank.get('ants_flag', False),
                    'Crossback': rank.get('crossback_flag', False),
                    'Power Trend': rank.get('power_trend', False),
                    'Mom Score': rank.get('clenow_score', 0),
                    'R²': rank.get('clenow_r2', 0),
                    'Liq_Expansion': rank.get('Liq_Expansion', False),
                })
                
            # Fetch Persistence Data
            from database import get_tml_persistence
            @st.cache_data(ttl=3600, show_spinner=False)
            def get_cached_persistence(market, days):
                from database import get_tml_persistence
                return get_tml_persistence(market, days=days)
            
            persistence_dict = get_cached_persistence("INDIA", 90)
            for d in display_data:
                clean_t = d['Ticker'].split()[0].replace('NSE:', '') + '.NS'
            ...
            if "Overview" in view_mode:
                ...
                show_cols = base_cols + ['EPS Gr %', 'Sales Gr %', 'ROE %', 'ADTV (Cr)', 'Breakdown Detail']
            elif "Institutional" in view_mode:
                show_cols = base_cols + ['Liq_Exp', 'UD_Ratio', 'HV1_Flag', 'PocketPivot', 'VDU_5D']
            else: # Technicals
                show_cols = base_cols + ['RMV_15D', '3WT', 'Ants', 'Crossback', 'Power Trend', 'Mom Score', 'R²', 'TML_Days']
            ...
            if 'RMV_15D' in df_render.columns:
                styled_df = styled_df.background_gradient(cmap='RdYlGn_r', subset=['RMV_15D'])
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "HV1_Flag": st.column_config.CheckboxColumn("🔊 HV1 Edge", help="Highest volume in 250 days"),
                    "UD_Ratio": st.column_config.NumberColumn("U/D Ratio (50d)", format="%.2f", help=">1.0 indicates institutional accumulation"),
                    ...
                    "RMV_15D": st.column_config.NumberColumn("RMV_15D", format="%.1f", help="Relative Measured Volatility (15-Day). 0-15 = Tight Coiling. 100 = High Volatility."),
            ...
            # Store leaders in session state for cross-referencing on other pages
            st.session_state['tml_leaders'] = final_leaders

            # =========================================================================
            # TOP MOMENTUM GAINERS
            # =========================================================================
            st.markdown("---")
            st.markdown("### 🚀 Top Momentum Gainers (Stage 2 Verified)")
            st.markdown("Displays the **Top 10** absolute percentage gainers from the list above over your selected timeframe.")
            
            gainer_timeframe = st.radio("Timeframe:", ["1 Week", "1 Month", "3 Months", "6 Months"], horizontal=True, key="gainer_timeframe")
            if gainer_timeframe == '1 Week': sort_key = 'Ret_1W'
            elif gainer_timeframe == '1 Month': sort_key = 'Ret_1M'
            elif gainer_timeframe == '3 Months': sort_key = 'Ret_3M'
            else: sort_key = 'Ret_6M'
            
            # Use final_leaders which is already loaded!
            gainer_list = list(final_leaders)
            gainer_list.sort(key=lambda x: x.get(sort_key, 0.0), reverse=True)
            top_10_gainers = gainer_list[:10]
...
.rank-2 { background: rgba(203, 213, 225, 0.15); color: #f8fafc; }
.rank-3 { background: rgba(217, 119, 6, 0.15); color: #f59e0b; }
...
<div class="gainer-grid">"""
                
                for idx, c in enumerate(top_10_gainers):
                    rank = idx + 1
                    clean_ticker = c['ticker'].replace('.NS', '')
                    price = c['close']
                    gain = c.get(sort_key, 0.0)
                    dist = c['dist_high']
                    chart_link = f"https://in.tradingview.com/chart/?symbol=NSE:{clean_ticker}"
                    
                    # Build Symbol Badges
                    ticker_display = clean_ticker
                    is_apex = (c.get('rs_score', 0) >= 95 and 
                               c.get('eps_growth', 0) >= 40 and 
                               c.get('sales_growth', 0) >= 40 and 
                               c.get('close', 0) > c.get('ema_21', 0))
                               
                    if is_apex: ticker_display += " 👑"
                    if clean_ticker in super_compounders: ticker_display += " ⚡"
                    if clean_ticker in rockets: ticker_display += " 🚀"
                    if c.get('rs_blue_dot', False): ticker_display += " 🔵"
                    if c.get('Is_Hold_The_Line', False): ticker_display += " 🛡️"
                    
                    if clean_ticker in recent_signals and recent_signals[clean_ticker]:
                        sig_name, sig_date = recent_signals[clean_ticker][0]
                        delta = (datetime.now() - datetime.strptime(sig_date, "%Y-%m-%d")).days
                        ago_str = "Today" if delta == 0 else f"{delta}d ago"
                        ticker_display += f" 🚨 <span style='font-size:0.65rem; color:#64748b; font-weight:600;'>{sig_name} ({ago_str})</span>"
...
<div class="gainer-card">
    <div class="g-header">
        <div class="g-ticker">{ticker_display}</div>
        <div class="g-rank {rank_class}">{rank_display}</div>
    </div>
    <div class="g-gain-pill">+{gain:.1f}%</div>
    <div class="g-stats">
        <div class="g-stat">
            <span class="g-stat-label">LTP</span>
            <span class="g-stat-value">₹{price:.2f}</span>
        </div>
        <div class="g-stat">
            <span class="g-stat-label">Off High</span>
            <span class="g-stat-value">{dist:.1f}%</span>
        </div>
    </div>
    <div class="g-footer">
        <div class="g-dist-badge">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
            Stage 2 Verified
        </div>
        <a href="{chart_link}" target="_blank" class="g-chart-link">
            TradingView ↗
        </a>
    </div>
</div>"""
                ...
                with st.expander("📋 Copy Top Gainers for TradingView"):
                    st.code(",".join([c['ticker'].replace('.NS', '') for c in top_10_gainers]), language="text")
            ...
            st.markdown("Real-time institutional narrative and catalysts for current True Market Leaders (Last 48 hours).")
            ...
            if active_tickers:
                try:
                    ...
                    with st.spinner("Scanning global feeds for TML catalysts..."):
...
"""
                        ...
                        st.info("No major catalysts or news stories detected for TML stocks in the last 48 hours.")
                except Exception as e:
                    st.error(f"📡 News radar temporarily offline. Error: {e}")
            else:
                st.info("No TML stocks available to scan for news.")
            ...
            try:
                ...
                with st.spinner("Fetching latest management interviews..."):
                    interviews = extract_portfolio_interviews(active_tickers, [])
...
"""
                    ...
                    st.markdown("### 🎙️ Media & Interviews")
                    st.info("No recent management interviews or updates were found for TML stocks on the major networks in the past 14 days.")
            ...
            with pin_cols[0]:
                pin_ticker = st.selectbox("Select Stock to Pin:", [d['Ticker'].split()[0] for d in display_data], key="pin_tml_in")
            with pin_cols[1]:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("⭐ Pin to Focus List", key="btn_pin_tml_in", use_container_width=True):
                    from database import add_to_focus_list
                    if add_to_focus_list(pin_ticker, "IN"):
            ...
            with c_liq1:
                liq_rs_filter = st.slider("Minimum RS Score", min_value=80, max_value=99, value=85, key="liq_rs_ind")
                
            filtered_liq = [x for x in final_leaders if x['rs_score'] >= liq_rs_filter]
            liquidity_leaders = sorted(filtered_liq, key=lambda x: x['adtv_cr'], reverse=True)[:15]
            ...
            if not liq_data:
                ...
                st.dataframe(
                    ...
                    column_config={
                        "Setup Status": st.column_config.TextColumn("Setup Status", help="Current Base Actionability"),
                        "Ticker": st.column_config.TextColumn("Ticker"),
                        "ADTV (Cr)": st.column_config.ProgressColumn("ADTV (Cr)", format="₹%d Cr", min_value=0, max_value=5000),
                        "TML Score": st.column_config.NumberColumn("TML Score", format="%.1f"),
                        "RS Score": st.column_config.ProgressColumn("RS", format="%.1f", min_value=0, max_value=99),
                        "U/D Vol": st.column_config.NumberColumn("U/D Vol", format="%.2f", help="Up/Down Volume Ratio"),
                        "Mom Score": st.column_config.NumberColumn("Mom Score", format="%.1f"),
                        "R²": st.column_config.NumberColumn("R²", format="%.2f"),
                        "RMV (15D)": st.column_config.NumberColumn("RMV_15D", format="%.1f"),
            ...
            from database import get_tml_hall_of_fame
            hof_data = get_tml_hall_of_fame("INDIA", days=90, limit=20)
            
            if hof_data:
                df_hof = pd.DataFrame(hof_data)
                df_hof['ticker'] = df_hof['ticker'].str.replace('.NS', '')
                df_hof['Chart'] = df_hof['ticker'].apply(lambda x: f"https://in.tradingview.com/chart/?symbol=NSE:{x}")
                ...
                def get_hof_ticker_display(t_clean):
                    ...
                    is_blue_dot = False
                    is_htt = False
                    if t_full in leaders_map:
                        rank = leaders_map[t_full]
                        is_apex = (rank['rs_score'] >= 95 and 
                                   rank.get('eps_growth', 0) >= 40 and 
                                   rank.get('sales_growth', 0) >= 40 and 
                                   rank['close'] > rank.get('ema_21', 0))
                    ...
                    display_val = f"NSE:{t_clean}"
                    if is_apex:
                        display_val += " 👑"
                    if t_clean in super_compounders:
                        display_val += " ⚡"
                    if t_clean in rockets:
                        display_val += " 🚀"
                    if is_blue_dot:
                        display_val += " 🔵"
                    if is_htt:
            ...
            if not plot_df.empty:
                if 'mcap_cr' not in plot_df.columns:
                    plot_df['mcap_cr'] = 0.0
                ...
                if asset_class != "ETFs":
                    ...
                    mcap_filter = st.radio(
                        ...
                        ["All", "Small Cap (< ₹5,000 Cr)", "Mid Cap (₹5,000 - ₹20,000 Cr)", "Large Cap (> ₹20,000 Cr)"],
                        horizontal=True,
                        label_visibility="collapsed"
                    )
                    
                    if mcap_filter == "Small Cap (< ₹5,000 Cr)":
                        plot_df = plot_df[plot_df['mcap_cr'] < 5000]
                    elif mcap_filter == "Mid Cap (₹5,000 - ₹20,000 Cr)":
                        plot_df = plot_df[(plot_df['mcap_cr'] >= 5000) & (plot_df['mcap_cr'] <= 20000)]
                    elif mcap_filter == "Large Cap (> ₹20,000 Cr)":
                        plot_df = plot_df[plot_df['mcap_cr'] > 20000]
                        
                plot_df['clean_ticker'] = plot_df['ticker'].str.replace('.NS', '')
                ...
                with tab_roppel:
                    ...
                    with col1:
                        color_by = st.radio("Color Bubbles By:", ["TML Score", "Industry"], horizontal=True, key="color_roppel")
                    with col2:
                        show_trails = st.toggle("☄️ Show RS Momentum Trails (Last 10 Days)", value=False, key="show_trails_in")
                    ...
                    if show_trails and not plot_df.empty:
                        ...
                        history_dict = get_historical_rs_for_tickers('INDIA', tickers_list, days=10)
                    ...
                    tv_list_dynamic = ",".join(["NSE:" + t for t in selected_tickers])
                    ...
                    st.info('''
                    **Chart Legend:**
                    - **Circle Size:** Institutional Liquidity (Average Daily Trading Value in Crores). Larger circles highlight mega/large-caps capable of absorbing massive mutual fund volume.
                ...
                with tab_power:
                    ...
                    **Chart Legend:**
                    ...
                    - **Circle Size:** Average Daily Trading Value (ADTV) in Crores.
                ...
                with tab_tq:
                    ...
                    tv_tq_list = ",".join(["NSE:" + t for t in tq_selected])
...
"""
    for idx, row in heat_df.iterrows():
        ...
        for t, crs in constits:
            ...
            tv_url = f"https://in.tradingview.com/chart/?symbol=NSE:{t}"
